...
var callLabel = document.querySelector("#callLabel");
...
var inputDevice = document.querySelector("#inputDevice");
var vapi;
var assistantId;
var sessionId;
var callActive = false;
var heardLocalAudio = false;
...
async function verifyMicrophone() {
  ...
  try {
    ...
    if (!track2 || track2.readyState !== "live") throw new DOMException("Microphone track is not live", "NotReadableError");
    const deviceId = track2.getSettings().deviceId || chosenDeviceId;
    await refreshInputDevices(deviceId);
    return { deviceId, label: track2.label || "microphone" };
  } catch (error) {
...
async function initialize() {
  ...
  vapi.on("speech-start", () => {
    status.textContent = "Ava is speaking";
  });
  vapi.on("speech-end", () => {
